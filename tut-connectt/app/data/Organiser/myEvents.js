...
// --- NEW: getAttendeeEvents (for Attendee discover page - public access) ---
export const getAttendeeEvents = async () => {
  try {
    // Public endpoint - no auth required
    const response = await fetch(`${API_URL}/events?page=1&pageSize=20`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    console.error('❌ Failed to load ATTENDEE events from API', e);
    return []; // Return empty array on failure
  }
};

// --- NEW: getAttendeeTickets (for Attendee to fetch their tickets) ---
export const getAttendeeTickets = async () => {
  try {
    const token = await AsyncStorage.getItem('ATTENDEE_JWT_TOKEN');
    console.log('Attendee token retrieved:', token ? 'Token exists' : 'No token found');
    if (!token) {
      console.error('Attendee auth token not found. Please login.');
      throw new Error('UNAUTHORIZED: Attendee auth token not found. Please login.');
    }

    const response = await fetch(`${API_URL}/tickets/me/tickets`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
    ...
    // Map backend ticket data to frontend fields
    return tickets.map((ticket) => ({
      id: ticket.id,
      eventId: ticket.eventId,
      eventName: ticket.event?.name || 'Event Name Missing',
      qrCodeUrl: ticket.qrcodeORurl?.[0] || null, // Use the first QR code URL
      status: ticket.redeemed ? 'Redeemed' : 'Active',
      type: ticket.type,
      price: ticket.price,
    ...
    console.error('❌ Failed to load ATTENDEE tickets from API', e);
    return []; // Return empty array on failure
  }
};

// --- NEW: getAttendeeRegistrations (for Attendee to fetch their registrations) ---
export const getAttendeeRegistrations = async () => {
  try {
    const token = await AsyncStorage.getItem('ATTENDEE_JWT_TOKEN');
    console.log('Attendee token retrieved for registrations:', token ? 'Token exists' : 'No token found');
    if (!token) {
      console.error('Attendee auth token not found. Please login.');
      throw new Error('UNAUTHORIZED: Attendee auth token not found. Please login.');
    }
    ...
    // Map backend registration data to frontend fields
    return registrations.map((registration) => ({
      id: registration.id,
      eventId: registration.eventId,
      eventName: registration.event?.name || 'Event Name Missing',
    ...
    console.error('❌ Failed to load ATTENDEE registrations from API', e);
    return []; // Return empty array on failure
  }
};
// ... (rest of myEvents.js remains the same) ...
...
export const getEvents = async () => {
  try {
    ...
    const response = await fetch(`${API_URL}/admin/events?page=1&pageSize=20`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
